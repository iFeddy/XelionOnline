...
cExecCheck "ExitGateClick"
	...
	cLinkTo( PlyHandle, "Rou", 5323, 4501 )
...
cExecCheck( "Menu" )
	if Option == 1 then
		if cPlayerExist( Player ) ~= nil then
			local AceCard = CardInfo["AceCard"]
			local Ace = cGetItemLot( Player, AceCard["Index"] )
			if Ace ~= 0 then
				StartAceRoulette(NPC, Player, Option)
			else
				cMobDialog( Var["MapIndex"], ChatInfo["PlayRouletteGame"]["NoCards"]["SpeakerIndex"], ChatInfo["ScriptFileName"], ChatInfo["PlayRouletteGame"]["NoCards"]["MsgIndex"] )
			end	
		end
...
function StartAceRoulette(NPCHandle, PlyHandle, OptionHandle)
cExecCheck( "StartAceRoulette")
	if NPCHandle == nil
	then
	...
	PlayRouletteGameInfo["OptionHandle"] = OptionHandle
	PlayRouletteGameInfo["PlayerHandle"] = PlyHandle
	
	local AceCard = CardInfo["AceCard"]
	cInvenItemDestroy( PlyHandle, AceCard["Index"], AceCard["Num"] )
